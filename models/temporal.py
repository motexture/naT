...
class ScaledSinusoidalEmbeddings(nn.Module):
    def __init__(self):
        super(ScaledSinusoidalEmbeddings, self).__init__()
    ...
    def forward(self, x, reference_frames=16):
        _, seq_len, d_model = x.shape
        pe = self._get_embeddings(reference_frames, d_model)
                
        if seq_len != reference_frames:
...
class TemporalTransformer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.positional_encoding = ScaledSinusoidalEmbeddings()
